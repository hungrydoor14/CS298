...
def greedy_actions(net1, net2, s_np, device):
    ...
    a1 = int(net1(s)[0].argmax())
    a2 = int(net2(s)[0].argmax())
    return a1, a2
...
def nash_actions(net1, net2, env, s_np, device):
    s = torch.tensor(s_np[None], dtype=torch.float32, device=device)
    q1 = net1(s)[0].cpu().numpy()  # (K,)
    q2 = net2(s)[0].cpu().numpy()  # (K,)
    # Build joint Q matrices via outer product: Q1[i,j] = q1[i], Q2[i,j] = q2[j]
    K = len(q1)
    Q1 = np.tile(q1[:, None], (1, K))  # player 1 only cares about their own action
    Q2 = np.tile(q2[None, :], (K, 1))  # player 2 only cares about their own action
...
def train(env, steps=STEPS, warmup=WARMUP, batch_size=BATCH_SIZE, lr=LR,
    ...
    for t in range(1, steps + 1):
        ...
        with torch.no_grad():
            v1_next = tgt1(snb_t).max(dim=1).values
            v2_next = tgt2(snb_t).max(dim=1).values

        y1 = r1b_t + (1 - done_t) * GAMMA * v1_next
        y2 = r2b_t + (1 - done_t) * GAMMA * v2_next

        Q1_pred = net1(sb_t)
        Q1_sa = Q1_pred.gather(1, torch.tensor(a1b, dtype=torch.int64, device=device).view(-1, 1)).squeeze(1)
        ...
        Q2_pred = net2(sb_t)
        Q2_sa = Q2_pred.gather(1, torch.tensor(a2b, dtype=torch.int64, device=device).view(-1, 1)).squeeze(1)
