...
# Grid / game
GRID_SIZE = 30
TRAIN_GRID_SIZES = [8, 10] # [8, 10, 12, 15]
TRAIN_MAX_STEPS = 100
...
EPS_DECAY = 2000
TARGET_UPDATE = 50
UPDATE_EVERY = 8       # only update network every N steps
EPOCHS = 300
EPS_PER_EPOCH = 100
EVAL_EPISODES = 5
FINAL_EVAL_EPISODES = 10
PROGRESS_PRINT_EVERY = 10
INCLUDE_REACHABILITY_CHANNELS = False
STATE_CHANNELS = 8 if INCLUDE_REACHABILITY_CHANNELS else 6
...
ENEMY_REACH_REDUCTION_REWARD = 1.5
WIN_BONUS = 6.0
LOSE_PENALTY = -6.0
DRAW_BONUS = 1.0
...
def encode_state(env, player=None):
    ...
    enemy = env._other_player(player)
    board = env.board

    channels = np.zeros((STATE_CHANNELS, env.size, env.size), dtype=np.float32)
    channels[0] = board == player
    channels[1] = board == enemy
    channels[2] = board == EMPTY
    channels[3] = board == WALL

    player_row, player_col = env.pos[player]
    enemy_row, enemy_col = env.pos[enemy]
    channels[4, player_row, player_col] = 1.0
    channels[5, enemy_row, enemy_col] = 1.0
    if INCLUDE_REACHABILITY_CHANNELS:
        channels[6] = env.reachable_empty_mask(player)
        channels[7] = env.reachable_empty_mask(enemy)

    return torch.tensor(channels, dtype=torch.float32, device=device)


class DQN(nn.Module):
    def __init__(self, in_channels=STATE_CHANNELS, action_dim=4):
        super().__init__()
    ...
    def forward(self, s):
        ...
        q_values = self.head(torch.cat([avg_pool, max_pool], dim=1))
        return q_values.squeeze(0) if squeeze else q_values
...
class ReplayBuffer:
    ...
    def push(self, s, a, r, s_next, next_legal_mask, done):
        ...
        if shape not in self.buckets:
            self.buckets[shape] = deque(maxlen=self.capacity)
        self.buckets[shape].append((s, a, r, s_next, next_legal_mask, done))

    def sample(self, batch_size):
        populated = [bucket for bucket in self.buckets.values() if bucket]
        bucket = random.choice(populated)
        if len(bucket) >= batch_size:
            return random.sample(list(bucket), batch_size)
        return random.choices(list(bucket), k=batch_size)
...
class TerritoryEnv:
    ...
    def _reachable_empty_info(self, player=None):
        current = self.active_player if player is None else player
        if current in self._reachability_cache:
            return self._reachability_cache[current]

        enemy = self._other_player(current)
        mask = np.zeros((self.size, self.size), dtype=np.float32)
        start = self.pos[current]
        queue = deque([start])
        visited = {start}
        count = 0
        ...
        self._reachability_cache[current] = (mask, count)
        return self._reachability_cache[current]

    def reachable_empty_mask(self, player=None):
        return self._reachable_empty_info(player)[0]

    def reachable_empty_count(self, player=None):
        return self._reachable_empty_info(player)[1]
    ...
    def step(self, action):
        ...
        enemy_reachable_before = self.reachable_empty_count(enemy)
        ...
        enemy_reachable_after = self.reachable_empty_count(enemy)
        enemy_reach_delta = enemy_reachable_before - enemy_reachable_after
        reward += ENEMY_REACH_REDUCTION_REWARD * (enemy_reach_delta / (self.size * self.size))
...
def train(epochs=EPOCHS, eps_per_epoch=EPS_PER_EPOCH):
    set_seed(SEED)
    train_envs = [TerritoryEnv(size=size, max_steps=TRAIN_MAX_STEPS) for size in TRAIN_GRID_SIZES]
    ...
    for epoch in range(epochs):
        ...
        for ep in range(eps_per_epoch):
            ...
            train_env = random.choice(train_envs)
            ep_loss = run_episode(train_env, agents, EPS)
            epoch_losses.append(ep_loss)
            if PROGRESS_PRINT_EVERY and (ep + 1) % PROGRESS_PRINT_EVERY == 0:
                print(f"  epoch {epoch + 1}/{epochs}: episode {ep + 1}/{eps_per_epoch}")

        avg_loss = float(np.mean(epoch_losses))
        losses.append(avg_loss)        

        eval_env = random.choice(train_envs)
        metrics = evaluate_policy(eval_env, agents, episodes=EVAL_EPISODES, max_steps=TRAIN_MAX_STEPS)
        print(
            f"Epoch {epoch + 1}/{epochs}, "
            f"eval size = {eval_env.size}, "
            f"eps = {EPS:.3f}, "
    ...
    return train_envs[-1], agents, losses
...
if __name__ == "__main__":
    ...
    print("Training complete. Running final 30x30 evaluation")
    rollout_env = TerritoryEnv(size=GRID_SIZE, max_steps=ROLLOUT_MAX_STEPS)
    metrics = evaluate_policy(rollout_env, agents, episodes=FINAL_EVAL_EPISODES, max_steps=ROLLOUT_MAX_STEPS)

    print(
        f"Final eval ({FINAL_EVAL_EPISODES} games, {ROLLOUT_MAX_STEPS} steps max) | "
        f"red win rate: {metrics['red_win_rate']:.3f}, "
    ...
    # ROLLOUT
    print("Building one 30x30 rollout for the viewer")
